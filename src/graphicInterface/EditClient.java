...
public class EditClient extends JFrame {
	...
	public EditClient(WizardHandler myNameIsTim, int id) {
		option1 = -1;
		option2 = -1;
		frame.setExtendedState(JFrame.MAXIMIZED_BOTH);
		frame.setUndecorated(true);
		...
		txtrNewClientInsertion.setBackground(Color.BLACK);
		txtrNewClientInsertion.setFont(new Font("Consolas", Font.PLAIN, 13));
		txtrNewClientInsertion.setText("View Client");
		panel_1.add(txtrNewClientInsertion);
		...
		panel.setVisible(true);
		frame.setVisible(true);
		cln=myNameIsTim.getClientFull(id);
		System.out.println(cln.getName());
		int addressId=myNameIsTim.getAddressIdThruClientId(id);
		adr=myNameIsTim.getAddressFull(addressId);
		int prostheticId=myNameIsTim.getProstheticIdThruClientId(id);
		prs=myNameIsTim.getProstheticFull(prostheticId);
